...
void main() {
  group('planAndroidBuild', () {
    test('signed build: key.properties → apk(release) + aab → cleanup', () {
      final steps = planAndroidBuild(const AndroidBuildConfig(
        workspace: '/w',
        signed: true,
    ...
    test('artifacts=apk skips the appbundle even when signed', () {
      ...
      expect(
        steps.where((s) => s.label.contains('appbundle')),
        isEmpty,
      );
    });

    test('artifact paths differ by signing', () {
      expect(androidArtifactPath(AndroidArtifact.apk, signed: true),
          endsWith('app-release.apk'));
      expect(androidArtifactPath(AndroidArtifact.apk, signed: false),
          endsWith('app-debug.apk'));
      expect(androidArtifactPath(AndroidArtifact.aab, signed: true),
