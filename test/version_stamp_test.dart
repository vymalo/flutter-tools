...
void main() {
  group('planVersionStamp', () {
    ...
    test('honours a custom project-dir', () {
      final steps = planVersionStamp(
        const VersionStampConfig(
            workspace: '/w', projectDir: 'app', buildNumber: '7'),
      );
    ...
    test('passes the marketing-version override through to the step', () {
      final patch = planVersionStamp(
        const VersionStampConfig(
            workspace: '/w', buildNumber: '7', marketingVersion: '2.5.0'),
      ).single as PatchVersionStep;
  ...
  group('PatchVersionStep execution', () {
    ...
    Future<String> stamp(String initial, String build,
        {String? marketing}) async {
    ...
    test('replaces an existing +build, keeping the marketing version',
        () async {
    ...
    test('throws with an override when there is no version: line at all',
        () async {
      expect(stamp('name: x\n', '1', marketing: '1.0.0'),
