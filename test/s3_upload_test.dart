...
void main() {
  group('s3-upload', () {
    test('plan is a single aws s3 cp with --no-progress', () {
      final steps = planS3Upload(const S3UploadConfig(
        file: '/w/app.aab',
        bucket: 'artifacts',
    ...
    test('endpoint flag injected for S3-compatible stores (MinIO)', () {
      ...
      expect(planS3Upload(c).whereType<RunStep>().single.args,
    ...
    test('presign args carry the expiry (+ endpoint when set)', () {
      expect(
        presignArgs(const S3UploadConfig(
            file: 'f', bucket: 'b', key: 'k', expiresIn: 3600)),
